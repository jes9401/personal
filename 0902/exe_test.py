...
from torch import utils, cuda, load, no_grad
...
def get_result(file_name):
    ...
    class MedNISTDataset(utils.data.Dataset):
    ...
    with no_grad():
        for test_data in test_loader:
            test_images = test_data[0].to(my_device)
...
# onefile로 생성하지않고 onedir로 생성하면 에러는 발생하지않지만 용량이 2배정도 늘어납니다.
# 또한 해당 에러를 구글링해서 여러 가지 방안을 적용해 보았지만 해결되지 않았습니다.
